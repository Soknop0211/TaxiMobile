...
public class Pix extends AppCompatActivity implements View.OnTouchListener {
    ...
    private ImageView mHandleView, clickme, selection_check, selection_back;
    ...
    private ImageView openImageImg;
    ...
    private final static String appBarTitlePix = "app_bar_title_pix";
    private final static String appBarBackgroundColor = "app_bar_background_color";
    ...
    public static void start(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            PermUtil.checkForCamara_WritePermissions(context, check -> {
                ...
                i.putExtra(appBarTitlePix, appBarTitle);
                i.putExtra(OPEN_CAMERA_FRONT, showFrontCamera);
                i.putExtra(appBarBackgroundColor, appBarColor);
                context.startActivityForResult(i, requestCode);
            });
        } else {
            Intent i = new Intent(context, Pix.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
     ...
     /*   Utility.setupStatusBarHidden(Pix.this);
        Utility.hideStatusBar(Pix.this);*/
        setContentView(R.layout.activity_main_lib_support);
        initialize();
    }
    ...
    private void initialize() {
        ...
        try {
            ...
            title = getIntent().getStringExtra(appBarTitlePix);
            //Todo percentage
        ...
        int appBarColor = getIntent().getIntExtra(appBarBackgroundColor, -1);
        if (appBarColor != -1) {
            colorPrimaryDark = appBarColor;
            findViewById(R.id.appBar_ui).setBackgroundColor(colorPrimaryDark);
        }

        TextView titleAppBar = findViewById(R.id.title_app_bar_pix);
        titleAppBar.setText(title);

        ImageView actionBackImg = findViewById(R.id.action_back_img);
        actionBackImg.setOnClickListener(v -> {
            finish();
        });
...
*/
        ...
        openImageImg = findViewById(R.id.open_gallery_img);
        //set camera
        ...
        updateImages();
        titleAppBar.setOnClickListener(v -> {
        ...
        if (initaliseadapter.getItemList().size() > 1) {
            defaultPreviewImage = initaliseadapter.getItemList().get(1).getContentUrl();
            if (!defaultPreviewImage.equals("")) {
            }
        }
        openImageImg.setOnClickListener(v -> {
            if (mBottomSheetBehavior.getState() != BottomSheetBehavior.STATE_EXPANDED) {
