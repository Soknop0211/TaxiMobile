...
class NumericKeyboard : FrameLayout {
    ...
    private fun initAttributes(context: Context, attrs: AttributeSet, defStyleAttr: Int) {
        ...
        keyHeight = attributes.getDimensionPixelSize(R.styleable.NumericKeyboard_keyHeight,
            defaultKeyHeight)
        keyTextSize = attributes.getDimensionPixelSize(R.styleable.NumericKeyboard_keyTextSize,
            defaultKeyTextSize).toFloat()
    ...
    private fun updateView(layout: View) {
        ...
        keyRemove.setOnTouchListener { view, event ->
            when (event.action) {
                MotionEvent.ACTION_DOWN -> handler.postDelayed(removeChar.also { it.view = view },
                    750)
