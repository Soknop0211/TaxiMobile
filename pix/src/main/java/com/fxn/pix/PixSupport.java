...
import com.fxn.CropOverlayView;
...
import com.fxn.utility.ui.FastScrollStateChangeListenerSupport;
...
public class PixSupport extends AppCompatActivity implements View.OnTouchListener {
    ...
    private FastScrollStateChangeListenerSupport mFastScrollStateChangeListener;
                ...
                switch (newState) {
                    case RecyclerView.SCROLL_STATE_DRAGGING:
                        ...
                        if (!Utility.isViewVisible(mScrollbar) && (recyclerView.computeVerticalScrollRange() - mViewHeight > 0)) {
                            mScrollbarAnimator = Utility.showScrollbar(mScrollbar, PixSupport.this);
                        }
                        break;
    ...
    private OnSelectionListener onSelectionListener = new OnSelectionListener() {
            ...
            if (LongSelection) {
                if (selectionList.contains(img)) {
                    ...
                    if (SelectionCount <= selectionList.size()) {
                        Toast.makeText(PixSupport.this, String.format(getResources().getString(R.string.selection_limiter_pix), selectionList.size()), Toast.LENGTH_SHORT).show();
                        return;
                    }
        ...
        public void OnLongClick(Img img, View view, int position) {
            if (SelectionCount > 1) {
                Utility.vibe(PixSupport.this, 50);
    ...
    public static void start(final Activity context, final int requestCode, final int selectionCount) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            PermUtil.checkForCamara_WritePermissions(context, new WorkFinish() {
                @Override
                public void onWorkFinish(Boolean check) {
                    Intent i = new Intent(context, PixSupport.class);
    ...
    private final static String MARGIN_TOP = "MARGIN_TOP";
    private final static String OPEN_CAMERA_FRONT = "OPEN_CAMERA_FRONT";

    public static void start(
            final FragmentActivity context,
            final int requestCode,
            final int selectionCount,
            final int marginTopAsPer,
            final boolean showFrontCamera
    ) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            PermUtil.checkForCamara_WritePermissions(context, check -> {
                Intent i = new Intent(context, PixSupport.class);
                i.putExtra(SELECTION, selectionCount);
                i.putExtra(MARGIN_TOP, marginTopAsPer);
                i.putExtra(OPEN_CAMERA_FRONT, showFrontCamera);
                context.startActivityForResult(i, requestCode);
            });
        } else {
            Intent i = new Intent(context, PixSupport.class);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Utility.setupStatusBarHidden(PixSupport.this);
        Utility.hideStatusBar(PixSupport.this);
    ...
    private float marginTopArg = 0.1f;
    private boolean isShowFrontCamera = false;
    ...
    private void initialize() {
        ...
        try {
            ...
            //Todo percentage
            marginTopArg = (float) getIntent().getIntExtra(MARGIN_TOP, 10) / 100;
            isShowFrontCamera = getIntent().getBooleanExtra(OPEN_CAMERA_FRONT, false);
            CropOverlayView cropOverlayView = findViewById(R.id.camera_overlay);
            cropOverlayView.setMarginTop(marginTopArg);
       ...
       /* mCamera.setOnTouchListener(new View.OnTouchListener() {
        ...
        selection_back = findViewById(R.id.selection_back_1111);
        ...
        TOPBAR_HEIGHT = Utility.convertDpToPixel(56, PixSupport.this);
                    ...
                    public Unit invoke(Bitmap bitmap) {
                        if (bitmap != null) {
                            ...
                            synchronized (bitmap) {
                                ...
                                //Todo override cropping
                                CropOverlayView cropOverlayView = findViewById(R.id.camera_overlay);

                                int overlayWidth = (int) (bitmap.getHeight() * cropOverlayView.getWidthRatio());
                                //Ratio of width
                                ...
                                //Margin top Margin * 2
                                int cropWidth = (int) (overlayWidth * 0.90f);
                                int cropHeight = (int) (cropWidth * (1.4f - marginTopArg));

                                int cropX = (int) ((bitmap.getWidth() * 0.5) - (cropWidth * 0.5));
                                int cropY = (int) (overlayWidth * marginTopArg);

                                //Todo override
//                                cropX = Math.round(cropOverlayView.getX()/cropWidth) * bitmap.getWidth();
//                                cropY = Math.round(cropOverlayView.getY()/cropHeight) * bitmap.getHeight();

                                int scale = Math.round((float) bitmap.getHeight() / (float) cropOverlayView.getHeight());

                                int newWidth = cropOverlayView.getWidth() * scale;

                                Bitmap cropped = Bitmap.createBitmap(bitmap, cropX, cropY, cropWidth, cropHeight);
//                                Bitmap cropped = Bitmap.createBitmap(bitmap, 0, 0, newWidth, bitmap.getHeight());

                                //Todo draw rect on image
                                /*cropped = bitmap;
...
//
                                int scale = Math.round((float) cropOverlayView.getWidth() / (float) cropOverlayView.getHeight());

                                canvas.drawRect(cropOverlayView.getLeft(), cropOverlayView.getTop(), cropOverlayView.getRight() * overlayWidth, cropOverlayView.getBottom() * overlayWidth,paint);
*/
                                File photo = Utility.writeImage(cropped);
    ...
    private void updateImages() {
        ...
        Cursor cursor = Utility.getCursor(PixSupport.this);
        ...
        for (int i = 0; i < limit; i++) {
            ...
            String dateDifference = Utility.getDateDifference(PixSupport.this, calendar);
    ...
    private void setBottomSheetBehavior() {
        ...
        mBottomSheetBehavior.setBottomSheetCallback(new BottomSheetBehavior.BottomSheetCallback() {
            ...
            public void onSlide(@NonNull View bottomSheet, float slideOffset) {
                Utility.manipulateVisibility(PixSupport.this, slideOffset,
                        instantRecyclerView, recyclerView, status_bar_bg,
                        topbar, bottomButtons, sendButton, LongSelection);
                if (slideOffset == 1) {
                    Utility.showScrollbar(mScrollbar, PixSupport.this);
    ...
    private void setViewPositions(float y) {
        ...
        mBubbleView.setY(handleY + Utility.convertDpToPixel((56), PixSupport.this));
